...
def sim_fparaconv(fpara,slocal,elocal):
    return simp_fparaconv(fpara,slocal,elocal)
#
def simp_fparaconv(fpara,slocal,elocal):
    # 
    ...
    for ni in range(fpara.shape[0]):
        ...
        if slocal == 0:
            ...
            #
            ll = -0.5*rlen - i*(wid/2)*np.cos(dip)    #% low-left ,ul
            lr =  0.5*rlen - i*(wid/2)*np.cos(dip)    #% low-right,ur
            ul = -0.5*rlen + i*(wid/2)*np.cos(dip)    #% up-left  ,ll
            ur =  0.5*rlen + i*(wid/2)*np.cos(dip)    #% up-right ,lr
            sdep = wid/2*np.sin(dip)+dep
            #
        elif slocal == 99:
            #

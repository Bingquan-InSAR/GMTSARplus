...
#
  ...
  if ($#argv < 2) then
    echo ""
    echo "Usage: intf_tops_notopo.csh intf.in batch_tops.config [path of script] [method]"
    ...
    echo "  outputs:"
    echo "    to ./intf_all"
    echo "  Modified by FWP, @SYSU, Guangzhou, the DEM data are not needed to prepared repeatly.."
    echo "  2020/04/14"
    exit 1
  endif
...
#
  ...
  #
  # option for unwrapping method, fls or snaphu
  # 
  if ($#argv > 3) then
     set method = $4
  else
...
# clean up
#
  # modified by FWP, for parallel processing based on a super master...
  # 
  # cleanup.csh topo
#
# make topo_ra
...
#    echo "DEM2TOPOPHASE.CSH - END"
   echo "DEM data is supposed to be ready before this script..."
#
#  shift topo_ra
#  
...
#
  foreach line (`awk '{print $0}' $1`)
    ...
    #
    echo " gmtsar_filter.csh $ref.PRM $rep.PRM $filter $dec $range_dec $azimuth_dec 0 $alpha $fftsize"
    gmtsar_filter.csh $ref.PRM $rep.PRM $filter $dec $range_dec $azimuth_dec 0 $alpha $fftsize
...
#
    ...
    if ($threshold_snaphu != 0 ) then
      ...
      echo ""
      echo "gmtsar_unwrap.py - START"
      echo "threshold_snaphu: $threshold_snaphu"
      if ($near_interp == 1) then
        # snaphu_interp.csh $threshold_snaphu $defomax $region_cut
        echo " gmtsar_unwrap.py $threshold_snaphu -i 1 -o unwrap.grd -m $method -update -snaphu_alg $snaphu_alg"
        gmtsar_unwrap.py $threshold_snaphu -i 1 -o unwrap.grd -m $method -update -snaphu_alg $snaphu_alg
      else
        # snaphu.csh $threshold_snaphu $defomax $region_cut
        echo " gmtsar_unwrap.py $threshold_snaphu -i 0 -o unwrap.grd -m $method -update -snaphu_alg $snaphu_alg"
        gmtsar_unwrap.py $threshold_snaphu -i 0 -o unwrap.grd -m $method -update -snaphu_alg $snaphu_alg
      endif
      echo "SNAPHU.CSH - END"
    else
      echo ""
    ...
    #
    # To compare a decimal number in cshell is a terrible experience.
    # added by Wanpeng Feng, @SYSU, Guangzhou, 2020/09/02
    #
    set flag_threshold_geocode = `echo $threshold_geocode|awk '{if ($1>=0) print 1; else print 0}'`
    #
    if ( $topo_phase == 1 &&  $flag_threshold_geocode == 1 ) then
      #
      ...
      gmtsar_geocode.csh $threshold_geocode $script
      #
    else if( $flag_threshold_geocode != 1 ) then
      echo "SKIP GEOCODING"
    else
  ...
  cd ../..
  if(-d intf_all/$ref_id"_"$rep_id) rm -rf intf_all/$ref_id"_"$rep_id 
  #
  # cleaning intf_folder to save space
  # added by FWP, @SYSU, Guangzhou, 2020/08/27
  # 
  rm intf/$ref_id"_"$rep_id/imag*.grd -f 
  rm intf/$ref_id"_"$rep_id/real*.grd -f
  #

...
ls ../$F/intf_all/*/corr_ll.grd > corr_list
stack.csh corr_list 1 corr.grd corr_std.grd
gmt grd2xyz corr.grd  -s > corr.xyz 

#heading and incidence
ls ../${F}/intf_all/*/*.PRM   | head -n 1   | sed 's|^\.\./|../../../|'   > ../${F}/intf_all/tmpm.filelist
pSAR_gmtsar_s1insar2roi_aws.py ../$F/intf_all T0 ../DATA/S1_ZIP_RAW 4
...
paste disp_list date | while IFS=$'\t' read -r grd_file date; do
    ...
    if [[ -f "$grd_file" ]]; then
        ...
        GMTSAR_proj_ra2ll.csh ../$F/topo/trans.dat "$grd_file" "${grd_file%.*}_ll.grd"
...
#range and azimuth
proj_ll2ra_ascii.csh ../$F/topo/trans.dat vel_ll.xyz vel_ra.xyz

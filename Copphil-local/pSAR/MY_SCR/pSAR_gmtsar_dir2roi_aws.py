...
if True:
    ...
    #
    if len(prms)==0:
        print(" ERR: no valid PRM was found!!!")
        sys.exit(-1) 
    ...
    #
    print( outdir+'/geo_'+mdate+'_'+sdate)
    ...
    _,info = pGMT5SAR.gmtsar_baseline('baseline.dat',mode=1)
    #
    if len(info)>=1:
    ...
    if zipfolder is not None and os.path.exists(zipfolder):
        #
        zips = glob.glob(zipfolder+'/S1*%s*.SAFE' % mdate)
        if len(zips)>0:
            track = pS1.s1safe2track(zips[0])
            aux = pS1.s1safe2aux(zips[0])
            xmlinfo, xmls = pS1.s1safe2swathxmlString(zips[0],pol='vv') 
            ...
            xmlinfo = []
            for czip in zips:
                cxmls, xmls = pS1.s1safe2swathxmlString(czip,pol='vv')

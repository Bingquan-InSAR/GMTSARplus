...
errormessage:
    echo ""
    echo "Usage: geocode.csh correlation_threshold [geocoding_script]"
    echo ""
...
echo remarked is $remarked
 ...
 gmtsar_proj_ra2ll.csh trans.dat corr.grd        corr_ll.grd     $script      
 gmt grdedit -D//"dimensionless"/1///"$PWD:t geocoded correlation"/"$remarked"      corr_ll.grd
 gmtsar_proj_ra2ll.csh trans.dat phasefilt.grd   phasefilt_ll.grd  $script    
 gmt grdedit -D//"radians"/1///"$PWD:t wrapped phase after filtering"/"$remarked"   phasefilt_ll.grd
 gmtsar_proj_ra2ll.csh trans.dat phase_mask.grd  phase_mask_ll.grd  $script   
 gmt grdedit -D//"radians"/1///"$PWD:t wrapped phase after masking"/"$remarked"     phase_mask_ll.grd
 gmtsar_proj_ra2ll.csh trans.dat display_amp.grd display_amp_ll.grd $script   
 gmt grdedit -D//"dimensionless"/1///"PWD:t amplitude"/"$remarked"                  display_amp_ll.grd
if (-e xphase_mask.grd) then
  gmtsar_proj_ra2ll.csh trans.dat xphase_mask.grd xphase_mask_ll.grd $script 
  gmt grdedit -D//"radians"/1///"$PWD:t xphase"/"$remarked"                            xphase_mask_ll.grd
  gmtsar_proj_ra2ll.csh trans.dat yphase_mask.grd yphase_mask_ll.grd $script 
...
if (-e phasefilt_mask.grd) then
  gmtsar_proj_ra2ll.csh trans.dat phasefilt_mask.grd phasefilt_mask_ll.grd $script

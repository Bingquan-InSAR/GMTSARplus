...
set master_prm = S1_$3_ALL_$4.PRM
set XMAX = `grep num_rng_bins $master_prm | awk '{print $3}'`
set yvalid = `grep num_valid_az $master_prm | awk '{print $3}'`
set num_patch = `grep num_patches $master_prm | awk '{print $3}'`
set YMAX = `echo "$yvalid $num_patch" | awk '{print $1*$2}'`
set SC = `grep SC_identity $master_prm | awk '{print $3}'`
set PRF = `grep PRF *.PRM | awk 'NR == 1 {printf("%d", $3)}'`
set region = 0/$XMAX/0/$YMAX
echo $region

set filter = $GMTSAR_HOME/gmtsar/filters/gauss5x5 #gauss5x5 has better result
#set filter = gauss5x3
set namearray =
set weightarray = 

...
# A bundle script to generate the interferogram from GMTSAR software and further export to STAMPS forfmat 
...
# Updated by Bingquan Li and Ling Chang on August 19, 2025
  ...
  if [[ $# -ne 2 ]]; then
    echo ""
    echo "Usage: gmtsar2stamps_ps.sh [step] [parameter_with_path_directory]"
    echo ""
    echo "  Script to pre-process SAR data and export to STAMPS format"
    echo ""
    echo "  example : gmtsar2stamps_ps.sh 1 /home/isya/3d_disp/param_INSAR_G2S.txt"
    echo ""
    ...
    echo "        7  Generate Interferogram (Real and Imaginary format)"
    echo "        8  Cut the interferograms based on ROI"
    echo ""
    echo "        GMTSAR2STAMPS (PS Method) --> "
    echo "        9 Create Amplitude Dispersion Index"
    echo "        10 Convert GMTSAR result to be able processed by STAMPS PS"
    echo ""
    echo ""
    exit 1
  fi
...
echo " "
   ...
   echo "--------------> Create Master-Slave network for PS mentod"
...
echo " "
echo "INSAR_G2S STEP ---->>" $step
echo "8  Cut the interferograms based on ROI (optional) [SM]"

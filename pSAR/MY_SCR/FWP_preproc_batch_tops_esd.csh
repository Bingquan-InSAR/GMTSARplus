    ...
    if($mode == 1) then
      ...
      foreach file (`awk '{print $0}' tmp.filelist`)
	    ...
	    wait
          ...
          if ($tmp_da > -1000 && $tmp_da < 1000) then
            echo " spectral_diversity $tmp_master $stem 0 $sharedir/filters/gauss25x7 > tmp"
            spectral_diversity $tmp_master $stem 0 $sharedir/filters/gauss25x7 > tmp 
          else
            echo " spectral_diversity $tmp_master $stem $tmp_da $sharedir/filters/gauss25x7 > tmp"
          ...
          if ($mode == 2) then
            ...
            #
            if (! -e $stem.refine_ashift.grd) then
              gmt blockmedian test -R0/$rmax/0/$amax -I800/200 | gmt greenspline -Gtest.grd -R0/$rmax/0/$amax -I800/200 -D1 -Cn900 -r -V
              #
              gmt grdfilter test.grd -D0 -Fg8000/2000 -Gtest2.grd -V -x20
              #
              gmt grd2xyz test2.grd -bo3 > test2.xyz
              # 
              #
              echo " GMTSAR: gmt surface test2.xyz -Ra.grd -Gtest_b.grd -T0.25 -C0.0005 -bi3"
              gmt surface test2.xyz -Ra.grd -Gtest_b.grd -T0.25 -C0.0005 -bi3          
              # echo " GMTSAR: gmt grdsample test2.grd -Gtest_b.grd -Ra.grd -nl"
              # gmt grdsample test2.grd -Gtest_b.grd -Ra.grd -nl
              echo " GMTSAR: gmt grdmath test_b.grd FLIPUD $spec_sep DIV 2 PI MUL DIV = res_shift.grd"
              gmt grdmath test_b.grd FLIPUD $spec_sep DIV 2 PI MUL DIV = res_shift.grd
              #
              if (-e res_shift.grd) then
                 #
                 echo " Progress: pSAR_gmtsar_ashift_median.py res_shift.grd $stem.update_ashift.grd"
                 pSAR_gmtsar_ashift_median.py res_shift.grd $stem.update_ashift.grd
                 #
              endif
            endif
            #
            # a constant shift is applied only...
            # modified by FWP, @SYSU, Guanghzou, 2020/04/23
            # 
            if ( -e res_shift.grd ) then
               set median_ashift = `gmt grdinfo res_shift.grd -L1 |grep "median" | awk '{print $3}'`
            else
               set median_ashift = "$res_shift"
            endif
            #
            if ("$median_ashift" == "") then
               set median_ashift = "0" 
            endif
            #
            # gmt grdmath a.grd res_shift.grd ADD = tmp.grd
            # echo " Progress: (W.F.) gmt grdmath a.grd $median_ashift ADD = tmp.grd , $res_shift for $stem"
            # gmt grdmath a.grd $median_ashift ADD = tmp.grd
            if (-e tmp.grd) then
               rm tmp.grd -f
            endif
            #
            if (-e $stem.update_ashift.grd) then
               echo " Progress: (W.F.) median of $stem.update_ashift.grd will be added to the azimuth shift GRD..."
               #
               set const_ashift = `gmt grdinfo $stem.update_ashift.grd -L1 |grep "median" | awk '{print $3}'`
               # gmt grdmath a.grd $const_ashift ADD = tmp.grd
               if (-e $stem.refine_ashift.grd) then
                  echo gmt grdmath a.grd $stem.refine_ashift.grd ADD = tmp.grd
                  gmt grdmath a.grd $stem.refine_ashift.grd ADD = tmp.grd
               else
                  echo gmt grdmath a.grd $stem.update_ashift.grd ADD = tmp.grd
                  gmt grdmath a.grd $stem.update_ashift.grd ADD = tmp.grd
               endif
            else
               echo " Progress: (W.F.) gmt grdmath a.grd $median_ashift ADD = tmp.grd , $res_shift for $stem "
               gmt grdmath a.grd $median_ashift ADD = tmp.grd
            ...
            mv tmp.grd a.grd
            #
            # to make a copy for comparison
            if (-f res_shift.grd) then
              cp res_shift.grd $stem.a_afterESD.grd
            endif
            #

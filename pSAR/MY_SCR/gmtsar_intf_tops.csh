...
#
  ...
  if ($#argv < 2) then
    echo ""
    echo "Usage: gmtsar_intf_tops.csh intf.in batch_tops.config geocoding_script [fls or snaphu] [alpha, 0.5 in default] [fftsize, 32 in default]"
...
if ($stage <= 1) then
#
# I don't think a cleanup is necessary for most of cases
# noted by FWP, @SYSU, Guangzhou, 2020/04/16
# 
#clean up
#cleanup.csh topo
#
...
#
  if ($topo_phase == 1) then
    ...
    ln -s ../raw/$master.LED .
    #
    # It is not necessary to re-do dem conversion from llt to rat
    # added by FWP, @SYSU, Guangzhou, 2020/08/12
    # 
    echo $PWD
    if (-f dem.grd) then
...
#  
    if ($shift_topo == 1) then
      echo " "
      echo " FWP-> OFFSET_TOPO - START"
      cd topo
      #
      #
      if ( -f topo_shift.grd ) then
        echo " "
        echo " FWP-> Congratulations! topo_shift.grd has been done."
        echo " "
      else
        ln -s ../raw/$master.SLC .
        # slc2amp.csh master.PRM 4 amp-$master.grd
        # slc2amp_MUL.csh master.PRM amp-$master-r.grd $filter $range_dec $azimuth_dec
        slc2amp_MUL.csh master.PRM amp-$master-r.grd $filter $dec 2 2
        offset_topo amp-$master-r.grd topo_ra.grd 0 0 16 topo_shift.grd
      endif 
      #
      cd ..
...
#
  foreach line (`awk '{print $0}' $1`)
    ...
    #
    echo " +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
    echo " Filtering process: filter.csh $ref.PRM $rep.PRM $filter $dec $range_dec $azimuth_dec 0 $alpha $fftsize"
    echo " +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
    #
...
#
    ...
    if ( $topo_phase == 1 ) then
      ...
      rm trans.dat
      ln -s  ../../topo/trans.dat .
      echo "threshold_geocode: $threshold_geocode $script"
      gmtsar_geocode.csh $threshold_geocode $script
    else if( $threshold_geocode < 0 ) then
      echo "SKIP GEOCODING"
    else
